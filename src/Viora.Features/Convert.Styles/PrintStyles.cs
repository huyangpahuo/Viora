            ...
            for (int gy = 0; gy < rows - 1; gy++)
            {
                for (int gx = 0; gx < cols - 1; gx++)
                {
                    ...
                    foreach (var (a, b, c) in tris)
                    {
                        ...
                        ImageOps.FillTriangle(outPx, outStride, w, h, a, b, c, px[i], px[i + 1], px[i + 2], shadeEdges: true);
                    }
                }
...
public sealed class HalftoneStage : StageBase
{
    ...
    public override Task<ImageProcessingContext> ExecuteAsync(ImageProcessingContext context, IProgress<StageProgress>? progress, CancellationToken ct)
    {
        ...
        for (int cy = 0; cy < (h + cell - 1) / cell; cy++)
        {
            for (int cx = 0; cx < (w + cell - 1) / cell; cx++)
            {
                int ccx = cx * cell + cell / 2, ccy = cy * cell + cell / 2;
                int sx = Math.Clamp(ccx, 0, w - 1), sy = Math.Clamp(ccy, 0, h - 1);
                double t = lut[blurred[sy * w + sx]] / 255.0;   // brightness
                double radius = cell * 0.72 * Math.Sqrt(1.0 - t);
                if (radius < 0.4) continue;
                ImageOps.FillCircle(outPx, outStride, w, h, ccx, ccy, radius, 20, 20, 22);
            }
            progress?.Report(new StageProgress(Name, 0, 1, (cy + 1) / (double)((h + cell - 1) / cell)));
        }
