...
Viora 是一个模块化的 Windows 桌面图像创作、转换与风格化工作室,基于 WPF/.NET 8 构建。它的第一个内置能力是 **Anime Vector** 转换预设——扁平几何二次元 / 矢量插画风格化引擎;其插件体系保证未来的所有能力都可以在不重构应用的前提下加入。
...
- **Anime Vector 预设** — 调色板坍缩为少量硬边扁平色块、块状阴影、区域合并与边界简化、可选深色描边。算法直接源自对真实参考图的逐项分析(见 `docs/00-inspection-and-plan.md`)。
- **完整转换工作流** — 拖放或选择器导入、实时预览、原图/结果分屏与并排对比、参数滑杆与重置、重新运行、进度显示与取消、PNG/JPEG/BMP 导出。
- **插件系统** — 基于清单的发现、版本兼容检查、隔离加载(可回收 `AssemblyLoadContext`)、界面内安装/启用/停用/卸载、能力注册门控、错误遏制:坏插件绝不拖垮宿主。
...
2. 选择 **Anime Vector** 预设;调整 *调色板数量*、*平滑强度*、*细节保留*、*阴影色块*、*描边强度*、*饱和度* —— 拖动时预览自动以预览分辨率重新运行。
3. 拖动分割手柄对比原图/结果(或并排显示)。
...
dotnet test Viora.sln             # 运行 29 个测试
